...
from pkt_loader import pkt_loader
import numpy as np
...
def run(device):
    ...
    with open(f"training_pkt_log.txt", "w") as log_file:
        ...
        dataset = pkt_loader(source_dir=r'G:\Coding\wifi_sensing\pkt_img')
        train_indices = []
        test_indices = []

        for i, (file_path, _) in enumerate(zip(dataset.img_files, dataset.img_lable)):
        ...
        for epoch in range(0, num_epochs):
            ...
            with torch.no_grad():
                ...
                if acc > best_acc:
                    best_acc = acc
                    torch.save(model.state_dict(), 'pkt_model.pth')
